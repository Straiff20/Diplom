...
> Recursive version of [fs.readdir](https://nodejs.org/api/fs.html#fs_fs_readdir_path_options_callback). Exposes a **stream ForAPI** and a **promise ForAPI**.
...
`const stream = readdirp(root[, options])` — **Stream ForAPI**
...
`const entries = await readdirp.promise(root[, options])` — **Promise ForAPI**. Returns a list of [entry infos](#entryinfo).
...
- Upgrading 2.x to 3.x:
    ...
    - Replaced callback ForAPI with promise ForAPI.
